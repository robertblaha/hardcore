...
function UpravKusy(e) {
	...
	//alert(ks.value);
	if (Number(prepocet.value) != 1) {
		var vysl = '', puv = '';
		puv = e.target.value;
		vysl = ZaokrouhliNahoru(CeskeCislo(e.target.value), prepocet.value);
		e.target.value = vysl;
		if (Number(puv) != Number(vysl)) {
			alert('Minimální expediční balení je ' + prepocet.value + 'ks.')
		}
	}
	console.debug('JCena: ' + Number(jcena.value))
	//ks.value = FormatCisla(e.target.value * prepocet.value, 0);
	ks.value = FormatCisla(e.target.value, 0);
	//cena.innerHTML = FormatCisla(Math.round(Number(e.target.value) * prepocet.value * Number(jcena.value), 2), 2);
	cena.innerHTML = FormatCisla(Math.round(Number(e.target.value) * Number(jcena.value), 2), 2);
...
function PridejZbozi() {
	console.debug('Pridavam zbozi');
	Pracuji();
	var zok = dojo.byId('zboziok');
	var index = 0;
	if (zok.value == 'running') {
		console.debug('cekam na overeni...');
		window.setTimeout('PridejZbozi()', 500);
		return;
	} else {
...
function OverZbozi(e) {
	dojo.byId('zboziok').value = 'running';
	Pracuji();
...
function ZobrazPlatby(e) {
	...
	dojo.query("tr." + doprava).forEach(
    	function(cil) {
    		//alert(dojo.attr(cil, 'value'));
            dojo.byId('error-debug').value = '1: pl-' + cil.id.substr(3) + '(' + cil.id + ')';
			if (dojo.byId('zpusobdopravy')) {
				dojo.byId('error-debug').value += '\n\r' + dojo.byId('zpusobdopravy').innerHTML;
			}
        	//cil.style.display = 'table-row';
        	cil.style.display = '';
        	if (dojo.byId('pl-' + cil.id.substr(3)) && dojo.byId('pl-' + cil.id.substr(3)).checked) {
	...
	if (!platna) {
		...
		dojo.query("input.platba").forEach(
	    	function(cil) {
	    		//alert(dojo.attr(cil, 'value'));
                        dojo.byId('error-debug').value = '2: pl-' + 'tr-' + cil.id.substr(3) + '(' + cil.id + ')';
