...
function FormatCisla(c, p) {
	return Number(c).toFixed(p).toString().replace(/\./, ',');
}
...
function ZaokrouhliNahoru(co, kolik) {
	var deleni = 0, zaok = 0;
	deleni = co/kolik;
	zaok = Math.round(deleni, 0);
	if (zaok < deleni) {
		zaok = zaok + 1;
	}
	return FormatCisla(Math.round(zaok * kolik, 0), 0);
}
...
dojo.addOnLoad(function(){
	...
	dojo.query("a:not(.fotka):not(.noloadinfo)").forEach(
        ...
        //zobrazeni debug informaci
        if (dojo.byId("debug")) {
            dojo.connect(dojo.byId("letopocet"), 'onclick', function () {
                                                                dojo.byId("debug").style.display = '';
                                                            }
            );
        }
 });
